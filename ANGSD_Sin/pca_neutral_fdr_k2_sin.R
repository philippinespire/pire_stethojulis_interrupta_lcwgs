...
cov_matrix_angsd <- as.matrix(read.table("angsd_pca_notrans_neutral_fdr.cov"))
...
PCA <- function(cov_matrix_angsd, 
  ...
  p <- ggplot(data = e, aes(x = PC1, y = PC2, color = Era, shape = Location)) +
    ...
    # scale_x_continuous(breaks = seq(-0.20, 0.2, by = 0.1), 
    #                    limits = c(-0.23, 0.20), 
    #                    # expand = c(0, 1.1),
    #                    labels = scales::number_format(accuracy = 0.1)) +  # Round to 1 decimal places
    # scale_y_continuous(breaks = seq(-0.8, 0.3, by = 0.1), 
    #                    limits = c(-0.81, 0.3), 
    #                    #expand = c(0, 0.5),
    #                    labels = scales::number_format(accuracy = 0.1)) +  # Round to 1 decimal places
    theme_classic() +
...
print(pca)
# outFile pattern
outFile_plot_pca <- paste0("plots/", spp_code, "_plot_pca_notrans_neutral_fdr_k2.png")  
...
## SAVE PCA TABLE ##
# Outfile pattern
outfile_pca_neu <- paste0("output/", spp_code, "_pnd_pca_table_snps_neutral.rds")
...
print(pca)
# outFile pattern
outFile_plot_pca <- paste0("plots/", spp_code, "_plot_pca_notrans_neutral_fdr_k2_leg.png")  
...
print(pca)
# outFile pattern
# outFile_plot_pca <- paste0("plots/", spp_code, "_plot_pca_notrans_subset2_neutral_fdr_k2.png")  
...
# Define output file
outFile_permanova_result <- paste0("./plots/", spp_code, "_pca_subset2_permanova.csv")
...
# Define output file
outFile_betdisp_result <- paste0("./plots/", spp_code, "_pca_subset2_betdisp.csv")
